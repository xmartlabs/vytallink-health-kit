...
class BuildETLInput(BaseModel):
    """Input contract for the ETL extraction use case."""
...
@dataclass(slots=True)
class BuildETLUseCase:
    """Extract a window of health data and save it as a tabular file (CSV/Parquet)."""

    health_data_provider: HealthDataProvider

    def execute(self, request: BuildETLInput) -> None:
        ...
        for day in health_data.days:
            ...
            # Fetch records, fallback to empty records if missing
            sleep_rec = health_data.sleep.get(iso_day, SleepRecord(date=day))
            hr_rec = health_data.heart_rate.get(iso_day, HRRecord(date=day))
            activity_rec = health_data.activity.get(iso_day, ActivityRecord(date=day))

            row = {
                "date": iso_day,
                "sleep_total_minutes": sleep_rec.total_minutes,
                "sleep_deep_minutes": sleep_rec.deep_minutes,
                "sleep_rem_minutes": sleep_rec.rem_minutes,
                "sleep_light_minutes": sleep_rec.light_minutes,
                "sleep_awake_minutes": sleep_rec.awake_minutes,
                "resting_hr_bpm": hr_rec.resting_bpm,
                "activity_steps": activity_rec.steps,
                "activity_calories": activity_rec.active_calories,
                "activity_exercise_minutes": activity_rec.exercise_minutes,
            }
            rows.append(row)

        df = pd.DataFrame(rows)

        output_lower = request.output_file.lower()
        if output_lower.endswith(".parquet"):
            df.to_parquet(request.output_file, index=False)
        else:
            df.to_csv(request.output_file, index=False)


class ChatWithHealthDataInput(BaseModel):
    """Input contract for the CLI chat."""
...
@dataclass(slots=True)
class ChatWithHealthDataUseCase:
    """Answer questions based on user health data context."""

    health_data_provider: HealthDataProvider
    narrative_generator: NarrativeGenerator

    def execute(self, request: ChatWithHealthDataInput) -> str:
        """Execute a single chat turn."""
        health_data = self.health_data_provider.fetch_window(
            end_date=request.end_date,
            days=request.days,
        )
        return self.narrative_generator.chat(
            question=request.question, health_data=health_data
        )
